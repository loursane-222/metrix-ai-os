...
import type { GenerateStockOperationCommandText } from "./stock-operation-command-resolver";
...
export const generateStockOperationCommandText: GenerateStockOperationCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY })).content;
