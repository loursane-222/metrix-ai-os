...
import type { GenerateCompanyGoalCreateCommandText } from "./company-goal-create-command-resolver";
...
export const generateCompanyGoalCreateCommandText: GenerateCompanyGoalCreateCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY })).content;
