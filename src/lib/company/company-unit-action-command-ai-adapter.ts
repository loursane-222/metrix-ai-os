...
import type { GenerateCompanyUnitActionCommandText } from "./company-unit-action-command-resolver";
...
export const generateCompanyUnitActionCommandText: GenerateCompanyUnitActionCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY })).content;
