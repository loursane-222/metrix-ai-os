...
import type { GenerateCompanyAssetCreateCommandText } from "./company-asset-create-command-resolver";
...
export const generateCompanyAssetCreateCommandText: GenerateCompanyAssetCreateCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY })).content;
