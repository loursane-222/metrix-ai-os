...
import type { GenerateCompanyProfileCandidateCommandText } from "./company-profile-candidate-command-resolver";
...
export const generateCompanyProfileCandidateCommandText: GenerateCompanyProfileCandidateCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY })).content;
