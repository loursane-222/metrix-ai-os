...
import type { GenerateCompanySourceCreateCommandText } from "./company-source-create-command-resolver";
...
export const generateCompanySourceCreateCommandText: GenerateCompanySourceCreateCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY })).content;
