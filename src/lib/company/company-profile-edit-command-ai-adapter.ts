...
import type { GenerateCompanyProfileEditCommandText } from "./company-profile-edit-command-resolver";
...
export const generateCompanyProfileEditCommandText: GenerateCompanyProfileEditCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY })).content;
