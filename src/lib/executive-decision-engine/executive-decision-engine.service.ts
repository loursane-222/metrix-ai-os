...
function buildOverdueDecisionCandidates(
  ...
  return [
    candidate({
      ...
      priority: "CRITICAL",
      title: `"${overdue.title}" kararının sonucunu bugün netleştir`,
      rationale: overdue.rationale,
      preferredFirstAction:
        overdue.actionHint ?? `"${overdue.title}" kararının sonucunu bugün netleştir.`,
      risks: ["Gecikmiş karar takibi diğer yönetim başlıklarını dağıtabilir."],
      impact: 96,
      urgency: 100,
      confidence: "HIGH",
      evidenceRefs: [`decision:${overdue.id}`],
      sourceSignals: ["Gecikmiş karar takibi"],
...
function buildDataQualityCandidates(
  ...
  return [
    candidate({
      ...
      priority: "CRITICAL",
      title: "Eksik veri kaynakları netleşmeden kesin yönetim yorumu yapma",
      rationale: `Operating context adimlari eksik: ${failedSteps.slice(0, 3).join(", ")}.`,
      preferredFirstAction: "Bugün eksik veri kaynaklarını ayır ve kesin olmayan yorumları sınırla.",
      risks: ["Eksik veri karar kalitesini düşürebilir."],
      impact: 88,
      urgency: 96,
      confidence: "LOW",
...
function buildAwarenessCandidates(
  ...
  return awareness.watchAreas.map((area) =>
    candidate({
      ...
      title: `${categoryLabel(AWARENESS_AREA_TO_DECISION[area])} alanını yönetim radarında tut`,
...
function buildFallbackCandidate(
  ...
  return candidate({
    ...
    confidence: "LOW",
    evidenceRefs: [],
    sourceSignals: ["Fallback yönetim kontrolü"],
    followUpWindow: "this week",
    isFallback: true,
  });
}
...
function buildOpportunities(category: ExecutiveDecisionCategory): string[] {
  const map: Record<ExecutiveDecisionCategory, string> = {
    ...
    DECISION_FOLLOW_UP: "Açık karar kapanırsa yönetim ritmi güçlenir.",
    MARKET: "Piyasa etkisi erken yansitilirsa fiyatlama ve nakit riski azalir.",
    DATA_QUALITY: "Veri netleşirse sonraki karar daha güvenilir olur.",
    STRATEGY: "Hedef netleşirse günlük öncelikler daha isabetli sıralanır.",
    PEOPLE: "Rol ve beklenti netleşirse ekip performansı daha ölçülebilir olur.",
    CUSTOMER: "Müşteri sahipliği netleşirse güven ve gelir korunur.",
  };
