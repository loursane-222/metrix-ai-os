...
export function resolveSupplierEditCommandRequest(supplierId: string, payload: { utterance: string; activeTab: string }): Promise<SupplierApiResult<{ outcome: unknown }>> { return supplierRequest(supplierId, "/actions/edit-command", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }); }
