...
import type { GenerateTaskEditCommandText } from "./task-edit-command-resolver";
...
export const generateTaskEditCommandText: GenerateTaskEditCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY_MEMORY_CONTEXT })).content;
