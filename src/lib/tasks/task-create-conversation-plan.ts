...
export type TaskCreatePlan =
  | { kind: "CREATE_PLAN"; intent: "OPEN" | "UPDATE_DRAFT" | "COMMIT" | "OPEN_UPDATE_COMMIT"; fields: TaskCreatePlanFields; explicitCommit: boolean; assigneeReference: string | null }
  | { kind: "STATUS_QUERY" }
  | { kind: "CANCEL" }
...
export function validateTaskCreatePlan(raw: unknown): TaskCreatePlan | null {
  ...
  if (!hasExactKeys(raw, ["kind", "intent", "fields", "explicitCommit", "assigneeReference"])) return null;
  ...
  if (raw.assigneeReference !== null && typeof raw.assigneeReference !== "string") return null;
  const assigneeReference = typeof raw.assigneeReference === "string" && raw.assigneeReference.trim() ? raw.assigneeReference.trim() : null;
  const fields: TaskCreatePlanFields = {};
  for (const [key, value] of Object.entries(raw.fields)) {
    // assigneeUserId is never planner-writable — it is resolved server-side
    // (create-command/route.ts) from assigneeReference via the canonical
    // member/self resolver, never invented by the LLM. See
    // "Planner Rule" / "Member / Assignee Authority" in the task-create
    // regression repair notes.
    if (key === "assigneeUserId") return null;
  ...
  return { kind: "CREATE_PLAN", intent: raw.intent as Extract<TaskCreatePlan, { kind: "CREATE_PLAN" }>["intent"], fields, explicitCommit: raw.explicitCommit, assigneeReference };
}
