...
function validateSemantic(raw: unknown): Extract<CustomerCreatePlan, { kind: "CREATE_PLAN" }>["semantic"] | null {
  ...
  if (raw.domain !== "customers" || typeof raw.stage !== "string" || !["HIGH", "MEDIUM", "LOW"].includes(String(raw.confidence)) || !["PROVIDER", "DETERMINISTIC"].includes(String(raw.source)) || typeof raw.fallbackUsed !== "boolean" || typeof raw.activeWorkflow !== "boolean") return null;
  if (raw.probableClauseCount !== undefined && (!Number.isInteger(raw.probableClauseCount) || Number(raw.probableClauseCount) < 0)) return null;
  return { domain: "customers", stage: raw.stage, confidence: raw.confidence as "HIGH" | "MEDIUM" | "LOW", source: raw.source as "PROVIDER" | "DETERMINISTIC", fallbackUsed: raw.fallbackUsed, activeWorkflow: raw.activeWorkflow, ...(typeof raw.probableClauseCount === "number" ? { probableClauseCount: raw.probableClauseCount } : {}) };
}
