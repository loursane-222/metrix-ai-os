...
export async function detectCustomerDuplicates(organizationId: string, values: Record<string, unknown>): Promise<CustomerDuplicateCandidate[]> {
  ...
  return rows.map((row) => { const matchedFields = (["taxNumber", "legalName", "cariKodu", "email", "phone"] as const).filter((key) => values[`customer.${key}`] !== undefined && row[key] === values[`customer.${key}`]); return { customerId: row.id, displayName: row.displayName, strength: matchedFields.some((key) => ["taxNumber", "legalName", "cariKodu"].includes(key)) ? "STRONG" : "WEAK", matchedFields }; });
}
