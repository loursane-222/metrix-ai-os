...
export const generateCustomFieldPlanText: GenerateCustomFieldPlanText = async ({ systemPrompt, userMessage }) => { const provider = process.env.AI_PROVIDER?.trim().toLowerCase() === "openai" ? openai : mockProvider; return (await provider.generateResponse({ systemPrompt, userMessage, context })).content; };
