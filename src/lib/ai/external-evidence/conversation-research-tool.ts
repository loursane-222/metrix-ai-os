...
export function buildExternalEvidencePromptLine(
  ...
  const freshnessNote =
    recency !== "any"
      ...
      : "";
  // Epistemic precision (Problem 2, independent of explicit recency): when
  // a capability's own adapter reports observedAt (currency's FX reference
  // date, weather's forecast day) that is the evidence's true date — never
  // "retrieved ${result.retrievedAt}", which is only when METRIX fetched it
  // just now. Production regression (2026-09-01): a same-day fetch of a
