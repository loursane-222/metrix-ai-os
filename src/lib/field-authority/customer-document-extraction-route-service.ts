...
export async function extractCustomerDocument(request: Request, deps: { extractor: StructuredFieldExtractionAdapter } = { extractor: customerDocumentFieldExtractor }): Promise<Response> {
  ...
  try {
    ...
    await prisma.customerDocumentAttachment.update({ where: { id: attachment.id, organizationId: auth.organization.id }, data: { extractionStatus: "COMPLETED", extractionPayload: payload as unknown as Prisma.InputJsonValue, extractedAt: new Date() } });
