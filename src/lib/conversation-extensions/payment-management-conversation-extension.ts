...
// Marking a payment as paid, and the AI-suggested collection-action/dunning
// workflow the constitution also describes, are deliberately not built here
// — the existing CollectionAction data model and Action Registry policy
// (collection.start/collection.set_lifecycle) are untouched, ready for a
// later operation to wire up the same way.
...
export const paymentManagementConversationExtension: ConversationExtension = {
  ...
  async execute(utterance, source = "written", correlationId = crypto.randomUUID()) {
    ...
    const createResult = await createPayment({ customerId: customer.id, title: `${customer.displayName} Tahsilatı`, amount });
    if (!createResult.ok) {
