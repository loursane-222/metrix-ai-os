...
import type { GenerateOrderEditCommandText } from "./order-edit-command-resolver";
...
export const generateOrderEditCommandText: GenerateOrderEditCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY_MEMORY_CONTEXT })).content;
