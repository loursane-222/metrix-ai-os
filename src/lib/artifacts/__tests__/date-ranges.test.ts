...
describe("resolvePreviousCalendarMonthRange — deterministic period boundaries", () => {
  it("resolves the full previous calendar month for a mid-month instant", () => {
    ...
    expect(period.from.toISOString()).toBe("2026-07-31T21:00:00.000Z");
    expect(period.to.toISOString()).toBe("2026-08-31T21:00:00.000Z");
  ...
  it("handles the January → previous December year rollover", () => {
    ...
    expect(period.from.toISOString()).toBe("2025-11-30T21:00:00.000Z");
    expect(period.to.toISOString()).toBe("2025-12-31T21:00:00.000Z");
