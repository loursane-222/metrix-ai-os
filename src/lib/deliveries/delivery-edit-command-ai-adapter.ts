...
import type { GenerateDeliveryEditCommandText } from "./delivery-edit-command-resolver";
...
export const generateDeliveryEditCommandText: GenerateDeliveryEditCommandText = async ({ systemPrompt, userMessage }) => (await (resolveConfiguredAiProvider() === "openai" ? provider : mockProvider).generateResponse({ systemPrompt, userMessage, context: EMPTY_MEMORY_CONTEXT })).content;
