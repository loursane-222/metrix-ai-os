...
type ParseResponse = {
  mapping: Record<string, SupplierImportField | "unmapped">;
...
const FIELD_LABELS: Record<SupplierImportField, string> = {
...
export function SupplierImportWizard() {
  ...
  async function handleFileSelected(file: File) {
    ...
    try {
      ...
      const response = await fetch("/api/suppliers/imports/parse", { method: "POST", credentials: "include", body: formData });
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      <header>
        <p className="text-xs uppercase tracking-[.18em] text-[#7C7466]">Tedarikçi İçe Aktarma</p>
        <h1 className="mt-1 text-xl font-semibold text-[#EDE7D9]">Excel/CSV&apos;den Tedarikçi Aktar</h1>
        <p className="mt-1 text-sm text-[#A79F91]">Başka bir programdan (Bizim Hesap, Logo, Mikro, Paraşüt vb.) dışa aktardığınız tedarikçi listesini yükleyin.</p>
      </header>
      ...
      {step === "preview" && preview ? (
        <section className="space-y-4">
          <div className="rounded-[20px] border border-white/[.08] bg-white/[.035] p-4 text-sm text-[#A79F91]">
            ...
            {preview.duplicateCount ? <p className="mt-1 text-[#f0b429]">{preview.duplicateCount} satır mevcut tedarikçiyle eşleşiyor — varsayılan olarak atlanır, isterseniz &quot;Güncelle&quot;ye çevirin.</p> : null}
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white/[.03] text-[#7C7466]">
                <tr>
                  ...
                  {(Object.keys(FIELD_LABELS) as SupplierImportField[]).map((field) => <th className="px-3 py-2" key={field}>{FIELD_LABELS[field]}</th>)}
              ...
              <tbody>
                {preview.rows.map((row) => (
                  <tr className={`border-t border-white/[.06] ${row.action === "skip" ? "opacity-40" : ""}`} key={row.rowIndex}>
                    ...
                    {(Object.keys(FIELD_LABELS) as SupplierImportField[]).map((field) => <td className="px-3 py-2 text-[#EDE7D9]" key={field}>{row.values[field] ?? "—"}</td>)}
                    <td className="px-3 py-2">{row.mergeTargetName ? <span className="rounded-full border border-[#f0b429]/30 bg-[#f0b429]/10 px-2 py-0.5 text-xs text-[#f0b429]">{row.mergeTargetName} ile eşleşme</span> : null}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {step === "done" && result ? (
        <section className="rounded-[20px] border border-[#34e6cf]/20 bg-[#34e6cf]/10 p-6 text-sm text-[#EDE7D9]">
          <p className="font-semibold">{result.created} yeni tedarikçi oluşturuldu{result.updated ? `, ${result.updated} tedarikçi güncellendi` : ""}.</p>
