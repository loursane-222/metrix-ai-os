...
import type { ProductionImportField } from "@/lib/imports/production-header-mapping";

type ProductMatch = { status: "RESOLVED"; id: string; label: string } | { status: "NOT_FOUND" } | { status: "AMBIGUOUS" };
...
type ParseResponse = {
  mapping: Record<string, ProductionImportField | "unmapped">;
...
const FIELD_LABELS: Record<ProductionImportField, string> = {
  orderNumber: "Emir No",
  productRef: "Ürün",
...
export function ProductionImportWizard() {
  ...
  async function handleFileSelected(file: File) {
    ...
    try {
      ...
      const response = await fetch("/api/production/imports/parse", { method: "POST", credentials: "include", body: formData });
  ...
  async function commit() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/production/imports/commit", {
        method: "POST",
        credentials: "include",
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      <header>
        <p className="text-xs uppercase tracking-[.18em] text-[#7C7466]">Üretim İçe Aktarma</p>
        <h1 className="mt-1 text-xl font-semibold text-[#EDE7D9]">Excel/CSV&apos;den Üretim Emri Aktar</h1>
        <p className="mt-1 text-sm text-[#A79F91]">Başka bir programdan (Bizim Hesap, Logo, Mikro, Paraşüt vb.) dışa aktardığınız üretim emri listesini yükleyin.</p>
      </header>
        ...
        <section className="space-y-4">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white/[.03] text-[#7C7466]">
                <tr>
                  ...
                  {(Object.keys(FIELD_LABELS) as ProductionImportField[]).map((field) => <th className="px-3 py-2" key={field}>{FIELD_LABELS[field]}</th>)}
                ...
                {preview.rows.map((row) => {
                  ...
                  return (
                    <tr className={`border-t border-white/[.06] ${row.excluded ? "opacity-40" : ""}`} key={row.rowIndex}>
                      ...
                      {(Object.keys(FIELD_LABELS) as ProductionImportField[]).map((field) => <td className="px-3 py-2 text-[#EDE7D9]" key={field}>{field === "productRef" && row.productMatch?.status === "RESOLVED" ? row.productMatch.label : row.values[field] ?? "—"}</td>)}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="rounded-xl bg-[#34e6cf] px-4 py-2.5 text-sm font-bold text-[#14120F] disabled:opacity-40" disabled={busy || includedRows.length === 0} onClick={() => void commit()} type="button">
              {busy ? "İçe aktarılıyor…" : `${includedRows.length} Kaydı İçe Aktar`}
            </button>
          </div>
        </section>
      ) : null}
      ...
      {step === "done" && result ? (
        <section className="rounded-[20px] border border-[#34e6cf]/20 bg-[#34e6cf]/10 p-6 text-sm text-[#EDE7D9]">
          <p className="font-semibold">{result.created} üretim emri oluşturuldu.</p>
