...
type ApiResponse = { ok: true } | { ok: false; code: string };

function messageForCode(code: string): string {
  if (code === "USER_ALREADY_HAS_ORGANIZATION") return "Bu hesapta zaten bir çalışma ortamı var.";
  return "Çalışma ortamı oluşturulamadı. Tekrar deneyin.";
}
...
export function OrganizationSetup({ contextError, onCreated }: { contextError: string | null; onCreated: () => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!result.ok) {
      setBusy(false);
      return setError(messageForCode(result.code));
    }
    await onCreated();
    setBusy(false);
  }
