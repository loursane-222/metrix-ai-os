...
export function EvidenceChain({ evidence, children }: { evidence: Array<{ evidenceId: string; summary: string; sourceDomain: string }>; children: ReactNode }) {
  const [open, setOpen] = useState(false); const items = evidence.filter((item) => item.evidenceId.trim().length > 0).slice(0, 5);
  if (!items.length) return <>{children}</>;
  return <div><button className="mt-2 text-xs text-[#c9bfa8] underline" onClick={() => setOpen((v) => !v)} type="button">{open ? "Kanıt yolunu kapat" : "Bu kanaate neden vardın?"}</button>{open ? <div className="mt-3 grid gap-2 border-l border-[#c9bfa8]/40 pl-3 transition-opacity duration-500 md:grid-cols-[1fr_42px_1fr] md:border-l-0 md:pl-0">{items.map((item, index) => <div key={item.evidenceId} className="relative rounded-lg border border-[#e4d6b6]/15 bg-[#1c1914] p-2 md:col-start-1"><p className="text-[10px] uppercase tracking-wider text-[#7c7466]">{item.sourceDomain} · {item.evidenceId}</p><p className="mt-1 text-xs text-[#ddd4be]">{item.summary}</p><span aria-hidden="true" className="pointer-events-none absolute left-full top-1/2 hidden h-px w-[42px] bg-[#c9bfa8]/35 md:block" />{index === items.length - 1 ? <div className="mt-2 border-t border-[#b8874a]/30 pt-2 text-[10px] uppercase tracking-wider text-[#b8874a] md:absolute md:left-[calc(100%+54px)] md:top-1/2 md:mt-0 md:w-[180px] md:-translate-y-1/2 md:border-t-0 md:border-l md:pl-3">Nihai kanaat</div> : null}</div>)}</div> : null}</div>;
}
