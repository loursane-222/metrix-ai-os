...
import type { PaymentImportField } from "@/lib/imports/payment-header-mapping";
...
type ParseResponse = {
  mapping: Record<string, PaymentImportField | "unmapped">;
...
const FIELD_LABELS: Record<PaymentImportField, string> = {
  customerRef: "Müşteri",
  title: "Açıklama",
  amount: "Tutar",
...
export function PaymentImportWizard() {
  ...
  async function handleFileSelected(file: File) {
    ...
    try {
      ...
      const response = await fetch("/api/payments/imports/parse", { method: "POST", credentials: "include", body: formData });
  ...
  async function commit() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/payments/imports/commit", {
        method: "POST",
        credentials: "include",
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      <header>
        <p className="text-xs uppercase tracking-[.18em] text-[#7C7466]">Tahsilat İçe Aktarma</p>
        <h1 className="mt-1 text-xl font-semibold text-[#EDE7D9]">Excel/CSV&apos;den Tahsilat Aktar</h1>
        <p className="mt-1 text-sm text-[#A79F91]">Başka bir programdan (Bizim Hesap, Logo, Mikro, Paraşüt vb.) dışa aktardığınız tahsilat/alacak listesini yükleyin.</p>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white/[.03] text-[#7C7466]">
                <tr>
                  ...
                  {(Object.keys(FIELD_LABELS) as PaymentImportField[]).map((field) => <th className="px-3 py-2" key={field}>{FIELD_LABELS[field]}</th>)}
                ...
                {preview.rows.map((row) => {
                  ...
                  return (
                    <tr className={`border-t border-white/[.06] ${row.excluded || note ? "opacity-40" : ""}`} key={row.rowIndex}>
                      ...
                      {(Object.keys(FIELD_LABELS) as PaymentImportField[]).map((field) => <td className="px-3 py-2 text-[#EDE7D9]" key={field}>{field === "customerRef" && row.customerMatch.status === "RESOLVED" ? row.customerMatch.customerName : row.values[field] ?? "—"}</td>)}
      ...
      {step === "done" && result ? (
        <section className="rounded-[20px] border border-[#34e6cf]/20 bg-[#34e6cf]/10 p-6 text-sm text-[#EDE7D9]">
          <p className="font-semibold">{result.created} tahsilat kaydı oluşturuldu.</p>
