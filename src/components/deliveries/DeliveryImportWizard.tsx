...
import type { DeliveryImportField } from "@/lib/imports/delivery-header-mapping";
import { chunkRows } from "@/lib/imports/chunk-rows";

type OrderMatch = { status: "RESOLVED"; orderId: string; orderNumber: string; customerId: string } | { status: "NOT_FOUND" };
...
type ParseResponse = {
  mapping: Record<string, DeliveryImportField | "unmapped">;
...
const FIELD_LABELS: Record<DeliveryImportField, string> = {
...
function matchNote(match: OrderMatch): string | null {
  if (match.status === "NOT_FOUND") return "Sipariş bulunamadı";
  return null;
}
...
export function DeliveryImportWizard() {
  ...
  async function handleFileSelected(file: File) {
    ...
    try {
      ...
      const response = await fetch("/api/deliveries/imports/parse", { method: "POST", credentials: "include", body: formData });
  ...
  async function commit() {
    ...
    const rows = preview.rows.filter((row) => !row.excluded && row.orderMatch.status === "RESOLVED");
    ...
    for (const chunk of chunks) {
      try {
        const response = await fetch("/api/deliveries/imports/commit", {
          method: "POST",
          credentials: "include",
  ...
  const includedRows = preview?.rows.filter((row) => !row.excluded && row.orderMatch.status === "RESOLVED") ?? [];

  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      <header>
        <p className="text-xs uppercase tracking-[.18em] text-[#7C7466]">İrsaliye İçe Aktarma</p>
        <h1 className="mt-1 text-xl font-semibold text-[#EDE7D9]">Excel/CSV&apos;den İrsaliye Aktar</h1>
        <p className="mt-1 text-sm text-[#A79F91]">Başka bir programdan (Bizim Hesap, Logo, Mikro, Paraşüt vb.) dışa aktardığınız irsaliye listesini yükleyin.</p>
        <p className="mt-1 text-sm text-[#A79F91]">Her satırın &quot;Sipariş No&quot; sütunu, METRIX&apos;te kayıtlı bir siparişle eşleşmelidir — eşleşmeyen satırlar otomatik atlanır.</p>
      </header>
      ...
      {step === "preview" && preview ? (
        <section className="space-y-4">
          ...
          <div className="rounded-[20px] border border-white/[.08] bg-white/[.035] p-4 text-sm text-[#A79F91]">
            ...
            {preview.unresolvedOrderCount ? <p className="mt-1 text-[#f0b429]">{preview.unresolvedOrderCount} satır sipariş eşleşmediği için atlandı.</p> : null}
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white/[.03] text-[#7C7466]">
                <tr>
                  ...
                  {(Object.keys(FIELD_LABELS) as DeliveryImportField[]).map((field) => <th className="px-3 py-2" key={field}>{FIELD_LABELS[field]}</th>)}
                  <th className="px-3 py-2">Not</th>
                </tr>
              </thead>
              <tbody>
                {preview.rows.map((row) => {
                  const note = matchNote(row.orderMatch);
                  return (
                    <tr className={`border-t border-white/[.06] ${row.excluded || note ? "opacity-40" : ""}`} key={row.rowIndex}>
                      <td className="px-3 py-2"><input checked={!row.excluded && row.orderMatch.status === "RESOLVED"} disabled={row.orderMatch.status !== "RESOLVED"} onChange={() => toggleRow(row.rowIndex)} type="checkbox" /></td>
                      {(Object.keys(FIELD_LABELS) as DeliveryImportField[]).map((field) => <td className="px-3 py-2 text-[#EDE7D9]" key={field}>{field === "orderNumberRef" && row.orderMatch.status === "RESOLVED" ? row.orderMatch.orderNumber : row.values[field] ?? "—"}</td>)}
      ...
      {step === "done" && result ? (
        <section className="rounded-[20px] border border-[#34e6cf]/20 bg-[#34e6cf]/10 p-6 text-sm text-[#EDE7D9]">
          <p className="font-semibold">{result.created} irsaliye kaydı oluşturuldu.</p>
