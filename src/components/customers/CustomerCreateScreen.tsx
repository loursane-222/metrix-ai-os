...
export function CustomerCreateScreen() {
  ...
  const universalRegistrations = useMemo<readonly UniversalRegistrationInput[]>(() => {
    ...
    return [
      { descriptor: { executiveTargetId: customerTargetId("create", "page", "customers"), authorityKey: "customers.create.page", targetKind: "page", module: "customers", entityType: "customer", label: "Yeni müşteri", readable: true, order: 0 }, adapter: {} },
      { descriptor: { executiveTargetId: surfaceId, authorityKey: "customers.customer.create", targetKind: "surface", module: "customers", entityType: "customer", label: "Müşteri oluşturma formu", readable: true, mutable: true, supportsDraft: false, order: 1 }, adapter: { validate: () => ({ valid: state.draft.displayName.trim().length > 0, missing: !state.draft.displayName.trim(), message: "Firma adı gerekli." }), commit: () => execute({ type: "commit" }), cancel: () => undefined } },
