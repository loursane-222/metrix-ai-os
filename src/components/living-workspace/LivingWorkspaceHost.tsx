...
export function LivingWorkspaceHost({ conversation }: { conversation?: React.ReactNode }) {
  ...
  return <AtmosphereAssessmentProvider>{preparingDomain ? <div aria-label="Çalışma alanı hazırlanıyor" className="pointer-events-none absolute inset-x-0 top-0 z-50 mx-auto h-px max-w-32 bg-[#34e6cf]/35 shadow-[0_0_10px_rgba(52,230,207,.18)]" data-executive-signature="sessiz.hazirlik" role="status" /> : null}<LivingWorkspaceSurface conversation={conversation} directive={directive} navigationCommand={navigationCommand} surfaceVisible={surfaceVisible} ready={ready} expanded={expanded} surfaceOpen={surfaceOpen} setSurfaceOpen={setSurfaceOpen} markSurfaceFailure={markSurfaceFailure} markSurfaceReady={markSurfaceReady} />
...
function LivingWorkspaceSurface({ conversation, directive, navigationCommand, surfaceVisible, ready, expanded, surfaceOpen, setSurfaceOpen, markSurfaceFailure, markSurfaceReady }: { conversation?: React.ReactNode; directive: WorkspaceDirective | null; navigationCommand: ReturnType<typeof executiveNavigationCommandRuntime.getSnapshot>; surfaceVisible: boolean; ready: boolean; expanded: boolean; surfaceOpen: boolean; setSurfaceOpen: (open: boolean) => void; markSurfaceFailure: () => void; markSurfaceReady: () => void }) {
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-hidden">
    {directive ? <section aria-label="Çalışma Alanı" aria-hidden={!surfaceVisible} className={`domain-workspace-stage overflow-hidden transition-[max-height,opacity,transform] duration-[380ms] ease-[cubic-bezier(.2,.8,.2,1)] motion-reduce:transition-none ${surfaceVisible ? "min-h-0 flex-1 border-b border-white/[.08] opacity-100" : "pointer-events-none max-h-0 shrink-0 opacity-0"}`} data-domain={directive.domain} data-executive-target="living-workspace" data-presentation-phase={!ready ? "loading" : surfaceOpen ? "open" : "opening"}>
      <div className={`metrix-atmosphere metrix-atmosphere-${atmosphereTone(assessment)} flex h-full min-h-0 w-full flex-col overflow-hidden bg-[#14120F]`} data-workspace-frame="in-flow-top">
        {!offerTemplate ? <div className="workspace-global-header shrink-0 px-3 pt-3 sm:px-5 sm:pt-4">
          <div className="mx-auto flex max-w-5xl items-center gap-3 rounded-[20px] border border-white/[.08] bg-white/[.035] px-3 py-2.5 md:border-x-0 md:border-t-0 md:bg-transparent md:shadow-none md:backdrop-blur-none">
            ...
            <div className="min-w-0 flex-1"><h1 className="truncate text-sm font-bold text-[#f4f7f8]">{directive.title}</h1><p className="mt-0.5 truncate text-[11px] text-[#93a0ad]">{workspaceIdentity(directive)}</p></div>
            <button aria-label="Günlük iş programını aç" className="rounded-xl border border-white/[.1] bg-white/[.04] px-3 py-2 text-xs font-semibold text-[#c9d1d6]" onClick={() => livingWorkspaceRuntime.publish(createCalendarWorkspaceDirective({ source: "system", correlationId: crypto.randomUUID() }))} type="button">Takvim</button>
          </div>
        </div> : null}
...
function GenericDirectiveSurface({ directive, onReady, onFailure }: { directive: WorkspaceDirective; onReady: () => void; onFailure: () => void }) {
  ...
  return <div className="mx-auto max-w-5xl">
    {state.status === "loading"
      ? <div className="mx-auto max-w-5xl rounded-[20px] border border-white/[.08] bg-white/[.035] p-4"><p className="text-sm font-semibold text-[#f4f7f8]">{directive.title}</p><p className="mt-1 text-xs text-[#93a0ad]">{workspaceIdentity(directive)} · Bilinen bilgiler hazırlanıyor…</p></div>
      : state.status === "error"
  ...
  return <div className="grid gap-3">{rows.map((row) => <Card key={String(row.id)}>
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0">
        <p className="text-sm font-semibold text-[#f4f7f8]">{String(row.title ?? "")}</p>
        {row.body ? <p className="mt-1 text-sm text-[#93a0ad]">{String(row.body)}</p> : null}
        <p className="mt-2 text-[10px] uppercase tracking-wider text-[#93a0ad]">{format(row.createdAt, "createdAt", undefined)}</p>
        {row.entityType === "Customer" && row.entityId ? <button className="mt-3 rounded-lg border border-[#34e6cf]/25 bg-[#34e6cf]/10 px-3 py-1.5 text-xs text-[#34e6cf]" onClick={() => void dispatchConversationNavigation({ route: `/metrix/customers/${encodeURIComponent(String(row.entityId))}`, source: "written", correlationId: crypto.randomUUID(), expectedSurfaceAuthorityKey: "customers.detail.page" })} type="button">Müşteri kaydını aç</button> : null}
      </div>
      {row.isRead ? null : <button className="shrink-0 rounded-lg border border-[#34e6cf]/25 bg-[#34e6cf]/10 px-3 py-1.5 text-xs text-[#34e6cf]" onClick={() => void markRead(String(row.id)).then(onRead)} type="button">Okundu işaretle</button>}
    </div>
  </Card>)}</div>;
}
