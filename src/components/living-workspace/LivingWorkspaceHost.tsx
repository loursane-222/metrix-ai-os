...
function SurfaceRenderer({ surface, data, onNotificationRead }: { surface: WorkspaceSurfaceDescriptor; data: unknown; onNotificationRead?: () => void }) {
  ...
  const record = data as { customers?: Array<Record<string, unknown>>; products?: Array<Record<string, unknown>>; notifications?: Array<Record<string, unknown>>; tasks?: Array<Record<string, unknown>>; payments?: Array<Record<string, unknown>> };
  if (surface.domain === "notification") return <NotificationListSurface rows={record.notifications ?? []} onRead={onNotificationRead}/>;
  let rows = record.customers ?? record.products ?? record.tasks ?? record.payments ?? [];
