...
export function LivingWorkspaceHost({ conversation }: { conversation?: React.ReactNode }) {
  ...
  return <div className="relative h-full min-h-0 overflow-hidden">
    ...
    {directive ? <section aria-label="Çalışma Alanı" aria-hidden={!surfaceVisible} className={`absolute inset-x-0 bottom-[76px] top-0 z-30 flex min-h-0 flex-col overflow-hidden bg-[#14120F] transition-opacity duration-[380ms] ease-[cubic-bezier(.2,.8,.2,1)] motion-reduce:transition-none md:inset-0 md:items-center md:justify-center md:bg-[#14120F]/35 md:px-12 md:py-8 md:backdrop-blur-[2px] ${surfaceVisible ? "pointer-events-auto" : "pointer-events-none"}`} data-executive-target="living-workspace" style={{ opacity: surfaceVisible ? 1 : 0 }}>
