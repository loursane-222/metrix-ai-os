...
export function CalendarWorkspace({ onReady, requestId, requestedView, requestedDate }: { onReady?: () => void; requestId?: string; requestedView?: "day" | "week" | "month"; requestedDate?: string }) {
  ...
  // Full 24-hour grid (00:00–23:00) — was hard-coded to 08:00–20:00, which
  // silently made anything outside that window (e.g. a real 21:30 event)
  // unreachable no matter what the data/truth layer returned. top/height
  // below are computed from this same 0-anchored, 48px/hour scale.
  const hours = Array.from({ length: 24 }, (_, index) => index);
  const timeBodyRef = useRef<HTMLDivElement>(null);
  // Deterministic scroll anchor, never a narration-specific hack: today's
  // real current time if the selected day is today, else the day's own
  // earliest real event, else the previous default (08:00) for an empty day.
  useEffect(() => {
    if (view === "month") return;
    const node = timeBodyRef.current;
    if (!node) return;
    const isToday = key(selected) === todayKey;
    const dayEvents = dayItems(selected).filter((entry) => !entry.allDay);
    const anchorHour = isToday
      ? new Date().getHours() + new Date().getMinutes() / 60
      : dayEvents.length > 0
        ? Math.min(...dayEvents.map((entry) => { const start = new Date(entry.dueDate); return start.getHours() + start.getMinutes() / 60; }))
        : 8;
    node.scrollTop = Math.max(0, (anchorHour - 2) * 48);
  }, [view, selected, items]);
  const timeline = (timelineDays: Date[]) => <div className="approved-calendar-timeline"><div className="approved-calendar-timeline-head"><span />{timelineDays.map((day) => <button className={key(day) === todayKey ? "is-today" : ""} key={day.toISOString()} onClick={() => setSelected(day)} type="button">{new Intl.DateTimeFormat("tr-TR",{weekday:"short",day:"numeric"}).format(day)}</button>)}</div><div className="approved-calendar-all-day-row"><strong>Tüm gün</strong>{timelineDays.map((day) => <div key={day.toISOString()}>{dayItems(day).filter((entry)=>entry.allDay).map(item)}</div>)}</div><div className="approved-calendar-time-body" ref={timeBodyRef}><div className="approved-calendar-gutter">{hours.map((hour)=><span key={hour}>{String(hour).padStart(2,"0")}:00</span>)}</div><div className="approved-calendar-time-columns">{timelineDays.map((day)=><div className={key(day)===todayKey?"is-today":""} key={day.toISOString()} onDoubleClick={()=>openCreate(day)} onDragOver={(event)=>event.preventDefault()} onDrop={(event)=>void move(event.dataTransfer.getData("text/calendar-event"),day)}>{dayItems(day).filter((entry)=>!entry.allDay).map((entry)=>{const start=new Date(entry.dueDate);const end=new Date(entry.endAt??new Date(start.getTime()+3_600_000));const top=Math.max(0,(start.getHours()+start.getMinutes()/60)*48);const height=Math.max(28,((end.getTime()-start.getTime())/3_600_000)*48);return <div className={entry.canonical?"approved-calendar-event is-canonical":"approved-calendar-event"} draggable={entry.canonical} key={`${entry.kind}:${entry.id}:${entry.dueDate}`} onDragStart={(event)=>entry.canonical&&event.dataTransfer.setData("text/calendar-event",entry.id)} style={{top,height}}><time>{new Intl.DateTimeFormat("tr-TR",{hour:"2-digit",minute:"2-digit"}).format(start)}</time><strong>{entry.title}</strong></div>})}</div>)}</div></div></div>;
