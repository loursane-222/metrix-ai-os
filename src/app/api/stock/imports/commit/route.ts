...
import { buildPropositionsFromReviewedRows, type ImportPreviewRow } from "@/lib/imports/stock-import.service";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    console.error("[stock_import_commit] failed", { errorName: error instanceof Error ? error.name : "UnknownError", errorMessage: error instanceof Error ? error.message : "Unknown error" });
...
function readImportRows(value: unknown): ImportPreviewRow[] {
  ...
  return value.map((item, index) => {
    ...
    if (typeof row.productMatch !== "object" || row.productMatch === null) throw new ApiValidationError(`rows[${index}].productMatch is required.`);
    if (typeof row.warehouseMatch !== "object" || row.warehouseMatch === null) throw new ApiValidationError(`rows[${index}].warehouseMatch is required.`);
    return {
      rowIndex: row.rowIndex,
      values: row.values as ImportPreviewRow["values"],
      productMatch: row.productMatch as ImportPreviewRow["productMatch"],
      warehouseMatch: row.warehouseMatch as ImportPreviewRow["warehouseMatch"],
