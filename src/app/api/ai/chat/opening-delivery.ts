...
export function createMetrixOpeningStream(input: {
  ...
  const systemPrompt = input.channel === "voice" && input.voiceAcknowledgement ? [
    ...
    "- Somut bir iş konusu bulunması tek başına açılış gerekçesi değildir. Basit, doğrudan cevaplanabilir bir bilgi sorusunda reflective giriş ekleme: HİÇBİR ŞEY üretme, tamamen boş çıktı ver. Her soruyu kapsamlı veya zor ilan etme.",
    "- Kullanıcı somut bir işlem/aksiyon istiyorsa (görev oluşturma/atama, teklif, fatura, tahsilat gibi bir kayıt oluşturma veya güncelleme, rapor/dosya üretme — gerçek bir mutasyon veya çok adımlı kanıt/araç çalışması gerektiren, birkaç saniye sürebilecek herhangi bir istek): kısa, tek cümlelik doğal bir açılış üret ki kullanıcı hangi işlemin ele alındığını bilsin. Yalnız hangi işlemi ele aldığını doğal biçimde belirt; sonucu, başarıyı veya tamamlandığını ASLA önceden söyleme.",
