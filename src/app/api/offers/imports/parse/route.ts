...
import { previewOfferImport } from "@/lib/imports/offer-import.service";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const preview = await previewOfferImport({ organizationId: auth.organization.id, headers, rows });
    const fileHash = hashFileContent(buffer);
    const priorImportAt = await findPriorFileImport(auth.organization.id, "Quote", fileHash);
    ...
    console.error("[offer_import_parse] failed", { errorName: error instanceof Error ? error.name : "UnknownError", errorMessage: error instanceof Error ? error.message : "Unknown error" });
