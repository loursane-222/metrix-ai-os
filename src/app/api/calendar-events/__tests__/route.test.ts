...
describe("GET /api/calendar-events — unified calendar truth (Workspace)", () => {
  ...
  it("C) native and Google events are both present in the same response, distinctly attributed", async () => {
    mocks.auth.mockResolvedValue(AUTH_CONTEXT);
    mocks.resolveCanonicalCalendarProjection.mockResolvedValue({
  ...
  it("E) exposes sourceStatuses honestly — an unavailable provider never masquerades as an empty-but-complete calendar", async () => {
    mocks.auth.mockResolvedValue(AUTH_CONTEXT);
    mocks.resolveCanonicalCalendarProjection.mockResolvedValue({ nativeEvents: [], googleEvents: [], sourceStatuses: { METRIX_NATIVE: "OK", GOOGLE: "UNAVAILABLE" } });
    const response = await callRoute();
    const body = await response.json();
    expect(body.data.sourceStatuses).toEqual({ METRIX_NATIVE: "OK", GOOGLE: "UNAVAILABLE" });
  });
  ...
  it("F) scopes the projection call to exactly the authenticated organization and user", async () => {
    ...
    mocks.resolveCanonicalCalendarProjection.mockResolvedValue({ nativeEvents: [], googleEvents: [], sourceStatuses: { METRIX_NATIVE: "OK", GOOGLE: "OK" } });
    await callRoute();
