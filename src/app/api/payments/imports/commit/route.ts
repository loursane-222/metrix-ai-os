...
import { buildPropositionsFromReviewedRows, type ImportPreviewRow } from "@/lib/imports/payment-import.service";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    console.error("[payment_import_commit] failed", { errorName: error instanceof Error ? error.name : "UnknownError", errorMessage: error instanceof Error ? error.message : "Unknown error" });
...
function readImportRows(value: unknown): ImportPreviewRow[] {
  ...
  return value.map((item, index) => {
    ...
    if (typeof row.customerMatch !== "object" || row.customerMatch === null) throw new ApiValidationError(`rows[${index}].customerMatch is required.`);
    return {
