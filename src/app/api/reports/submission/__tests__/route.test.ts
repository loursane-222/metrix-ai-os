...
const { requireAuthContextFromCookiesMock, processReportSubmissionMessageMock } = vi.hoisted(() => ({
  requireAuthContextFromCookiesMock: vi.fn(),
  processReportSubmissionMessageMock: vi.fn(),
}));
...
vi.mock("@/lib/reports/report-submission-orchestrator.service", () => ({
  processReportSubmissionMessage: processReportSubmissionMessageMock,
}));
...
function buildRequest(body: Record<string, unknown> | string): Request {
  return new Request("http://localhost/api/reports/submission", {
    method: "POST",
...
describe("POST /api/reports/submission", () => {
  beforeEach(() => {
    requireAuthContextFromCookiesMock.mockReset().mockResolvedValue(FAKE_AUTH_CONTEXT);
    processReportSubmissionMessageMock.mockReset();
  });

  it("rejects an empty message with 400", async () => {
    const response = await POST(buildRequest({ message: "   " }));
    expect(response.status).toBe(400);
    expect(processReportSubmissionMessageMock).not.toHaveBeenCalled();
  });
  ...
  it("passes the trusted server auth context and message through to the orchestrator", async () => {
    processReportSubmissionMessageMock.mockResolvedValue({ status: "SUBMITTED" });
    await POST(buildRequest({ message: "haftalık raporumu gönder" }));
    expect(processReportSubmissionMessageMock).toHaveBeenCalledWith({ authContext: FAKE_AUTH_CONTEXT, message: "haftalık raporumu gönder" });
  });

  it("returns the orchestrator result wrapped as { report } on success", async () => {
    const report = { status: "SUBMITTED", templateName: "Haftalık Yönetim Raporu", answeredNow: ["important_development"] };
    processReportSubmissionMessageMock.mockResolvedValue(report);
  ...
  it("maps an unrecognized thrown error to a safe generic 500", async () => {
    processReportSubmissionMessageMock.mockRejectedValue(new Error("internal db explosion, host=10.0.0.5"));
    const response = await POST(buildRequest({ message: "x" }));
