...
import { previewInvoiceImport } from "@/lib/imports/invoice-import.service";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const preview = await previewInvoiceImport({ organizationId: auth.organization.id, headers, rows });
    return ok(preview);
  } catch (error) {
    if (error instanceof UnsupportedSpreadsheetFileError) return fail(error.message, 400);
    if (error instanceof AuthError) return fail(error.message, error.status);
    console.error("[invoice_import_parse] failed", { errorName: error instanceof Error ? error.name : "UnknownError", errorMessage: error instanceof Error ? error.message : "Unknown error" });
