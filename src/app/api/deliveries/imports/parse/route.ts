...
import { previewDeliveryImport } from "@/lib/imports/delivery-import.service";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const preview = await previewDeliveryImport({ organizationId: auth.organization.id, headers, rows });
    const fileHash = hashFileContent(buffer);
    const priorImportAt = await findPriorFileImport(auth.organization.id, "Delivery", fileHash);
    ...
    console.error("[delivery_import_parse] failed", { errorName: error instanceof Error ? error.name : "UnknownError", errorMessage: error instanceof Error ? error.message : "Unknown error" });
