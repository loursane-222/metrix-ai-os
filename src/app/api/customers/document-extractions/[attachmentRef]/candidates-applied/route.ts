...
export async function POST(request: Request, context: { params: Promise<{ attachmentRef: string }> }): Promise<Response> {
  try {
    ...
    await prisma.customerDocumentAttachment.update({ where: { id: attachment.id, organizationId: auth.organization.id }, data: { reviewStatus: "READY", reviewPayload: reviewPayload as Prisma.InputJsonValue, draftId, correlationId } });
