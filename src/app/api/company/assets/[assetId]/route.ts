...
export async function PATCH(request: Request, context: { params: Promise<{ assetId: string }> }) {
  try {
    ...
    const security = await authorizeLegacyMutation({ authContext: auth, actionName: "company.asset.update", requiredPermission: "company.write", entityType: "CompanyAsset", entityId: assetId });
