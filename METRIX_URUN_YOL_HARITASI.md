...
- **Madde 8 — KISMEN İLERLEDİ (2026-08-27).** Ayarlar ekranı denetlendi: mevcut hâli yalnızca "Metrix Filmi" + "Çıkış Yap" idi, roadmap'te adı geçen 3 alt-başlığın (ses seçimi, dark/light, hesap ayarları) hiçbiri yoktu. **Hesap ayarları (ad-soyad, e-posta, saat dilimi) tamamlandı ve deploy edildi** (commit `b711873`) — yeni `GET/PATCH /api/user/profile`, `SettingsMenu`'ye "Hesap Ayarları" girişi. Kalan 2 alt-başlık hâlâ açık: **ses seçimi** (altyapı var — `voice-preference-authority.ts` — ama tek global env var'a bağlı, kullanıcı/organizasyon başına DB alanı+API+UI gerekiyor), **dark/light** (hiçbir altyapı yok, tüm renkler hardcoded — Madde 6'nın dışında ayrı ele alınacaksa önce bu karar netleşmeli, çünkü Codex/Gemini'nin dıştaki görsel yeniden tasarımıyla çakışma riski var).
  - **Ses seçimi — KAPANDI (2026-08-28).** `User.voicePreference` (nullable, timezone/language ile aynı desen) eklendi, `resolveVoiceAuthorityForUser` kullanıcının kayıtlı tercihini env varsayılanının önüne koyuyor, Hesap Ayarları'na "Ses" dropdown'u eklendi. Canlıda uçtan uca doğrulandı: kaydetme, sayfa yenilemesi sonrası kalıcılık, gerçek TTS/realtime session route'larının doğru sesi kullanması. Commit `947c245`.
  - **dark/light — KAPANDI, kapsam daraltılarak (2026-08-28, Murat'ın kararı).** Toggle yapılmayacak — METRIX yalnızca dark mod. `globals.css`'de Next.js'in varsayılan starter'ından kalma, gerçekte hiç kullanılmayan light/dark `:root` geçişi (`prefers-color-scheme: dark` medya sorgusu) temizlendi; sistem light modda olsa bile artık ilk boyamada beyaz flaş yok. Canlıda, tarayıcı renk şeması zorla light yapılarak doğrulandı — uygulama tamamen dark render ediyor. Commit `ff8475f`. Madde 8 bütünüyle kapandı.
...
- **Madde 2 — incelendi, kod değişikliği yapılmadı (Murat'ın izniyle: "kalite kaybı olmadan yapabiliyorsak yapalım").** `useVoiceChatConnection.ts`'deki gerçek WebRTC bağlantı sırası satır satır okundu. Tek gerçek paralelleştirme adayı — mikrofon izni (`getUserMedia`) ile OpenAI realtime session oluşturma çağrısının (`/api/ai/chat/voice/session`) art arda değil eşzamanlı çalıştırılması — incelendi ve reddedildi: (a) izin zaten verilmişse `getUserMedia` neredeyse anında döner, kazanç ihmal edilebilir; (b) izin ilk kez isteniyorsa paralelleştirme, kullanıcı izni reddettiğinde gerçek bir OpenAI realtime session'ının (ve VOICE_SESSION_CREATED rate-limit hakkının) boşa harcanmasına yol açar — bu bir maliyet/kötüye kullanım koruması, kaldırılması "kalite kaybı" ile aynı kategoriye giriyor. Sunucu tarafında da benzer bir durum var: rate-limit DB sorgusuyla OpenAI çağrısını paralelleştirmek aynı korumayı zayıflatır. Sonuç: önceki oturumun "kolay kazanımlar tükendi" tespiti doğrulandı, güvenli/bedava bir kazanım bulunamadı. Gerçek ilerleme hâlâ roadmap'in kendi önerdiği yolu gerektiriyor: açılış cümlesi için ayrı/daha hızlı bir modelin kimlik ve kaliteyi koruyup korumadığını test eden ayrı bir araştırma turu.
- **Madde 7 — Murat'ın talimatıyla beklemede**, ayrıca not: `BIZIMHESAP_PARTNER_KEY` gerektiren gerçek bir ortaklık ön koşulu var (`bizimhesap-client.ts`) — bu hazır olmadan webhook/canlı senkron kodu yazmak spekülatif olur.
